export default class GridViewLoader {
    ...
    get url() {
        return this.filter == 'igloo' ? '/client/media/igloos/buildings/icon' : '/client/media/furniture/icon'
    }
    ...
    loadItem(item, scale) {
        ...
        let url = this.filter == 'igloo' ? `${this.url}/${item}.webp` : `${this.url}/${scale}/${item}.webp`
