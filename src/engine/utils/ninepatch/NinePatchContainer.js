...
export default class NinePatchContainer extends Phaser.GameObjects.Container {
    ...
    redraw() {
        ...
        if (!this._originTexture.frames[this.textureFrame]) return
        this._originFrame = this._originTexture.frames[this.textureFrame]
    ...
    drawPatches() {
        ...
        const finalXs = this.flipX ? [this.width - this.marginLeft, this.marginRight, 0] : [0, this.marginLeft, this.width - this.marginRight]
        const finalYs = this.flipY ? [this.height - this.marginTop, this.marginBottom, 0] : [0, this.marginTop, this.height - this.marginBottom, this.height]
    ...
    setOrigin(x, y) {
        this.setNinePatchContainerOrigin(x, y)
        return this
    }
}
